...
useEffect(()=>{
  ...
  return (
    ...
    <div className='searchWrap'>
      <div id="searchId" className='seachFrom'>
        <input onFocus={seachInputClicks} onBlur={seachInput} value={search} onChange={inputChange} className='searchInput' placeholder='궁금한 내용을 검색해보세요.' />
        <button onClick={click} className="searchBtn" data-text="" style={{ backgroundImage: `url("https://image.dalbitlive.com/common/icon/searchBtn.png")` }}></button>
      </div>
        ...
        <div className='faqWrap'>
          {data.map((item, idx) =>{
            ...
            return (
              ...
              <div key={idx} onClick={() => {handleClick(item.list)}} className='faqList' date-index={item.list}>
                <div className='questionArea' key={item.list}>
                  <span className='qmark'>Q</span>
                  <span className='question'>{item.question}</span>
                  <span className={item.list === activeIndex ? 'arrow down' : 'arrow up'}></span>
                </div>
                <div className={item.list === activeIndex ? 'answerArea' : 'closed'}>
                  <p className={item.list === activeIndex ? 'answerText' : 'close'}>
                    {item.text}
                  </p>
                </div>
              </div>
